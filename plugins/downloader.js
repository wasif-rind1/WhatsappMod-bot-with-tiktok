...
async ({
    ...
    try {
        let match = args || m.quoted?.text;
        if (!match) return await m.reply(lang.NEED_URL);
        if (!(await isUrl(match))) return await m.reply(lang.INVALID_URL);
        await m.react('⬇️');
        const { data } = await getJson(config.API + "/api/downloader/tiktok?url=" + encodeURIComponent(match));
        await m.sendFromUrl(data.video, { caption: data.title || lang.DOWNLOADING });
        await m.react('✅');
    } catch (error) {
        await m.react('❌');
        console.error(error);
...
Sparky({
    ...
    fromMe: isPublic,
    category: "downloader",
    desc: "Download audio from a TikTok video by providing a valid URL",
},
async ({
    m, client, args
}) => {
    try {
        ...
        await m.react('⬇️');
        const { data } = await getJson(config.API + "/api/downloader/tiktok?url=" + encodeURIComponent(match));
        await m.sendMsg(m.jid, data.audio, { mimetype: "audio/mpeg" }, "audio");
        await m.react('✅');
    } catch (error) {
        await m.react('❌');
        console.error(error);
